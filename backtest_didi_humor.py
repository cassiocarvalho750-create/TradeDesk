...
def carrega_sp500(caminho):
    """Le o CSV do S&P 500 no mesmo formato das cestas e devolve uma Series
    booleana 'bom_humor' indexada por data: True quando, no S&P 500, a
    media 3 do Didi esta acima da media 8 (didi3 > 0)."""
...
def baixa_sp500():
    ...
    return (didi3 > 0), didi3

def humor_na_data(bom_humor, didi3_sp, data):
    ...
    caem em um mercado e nao no outro). Retorna (bom?, valor_didi3):
    ...
    if len(validos) == 0:
        return None, None
    ref = validos[-1]
    v = bom_humor.loc[ref]; d3 = didi3_sp.loc[ref]
    if pd.isna(v) or pd.isna(d3):
        return None, None
    return bool(v), float(d3)

# ------------------------------------------------------- backtest com humor
def backtest_com_humor(d, tk, bom_humor, didi3_sp, max_hold=120):
    ...
    while i < n - 1:
        ...
        r = 0.5 * p1 + 0.5 * p2
        humor, d3 = humor_na_data(bom_humor, didi3_sp, data_ent)  # True=bom, False=mau, None=sem dado
        trades.append({"ticker": tk, "data": data_ent, "R": r, "humor": humor, "didi3_sp": d3})
        i = saiu_em + 1
    return trades
...
def main():
    ...
    todos = []
    for tk, d in dfs:
        try: todos += backtest_com_humor(d, tk, bom_humor, didi3_sp)
